...
import {
    ...
    TrendingDown, RefreshCcw, Share2
} from 'lucide-react';
...
// --- CHART COMPONENTS (Copied from example.tsx) ---
                ...
                <div className="flex-1 flex flex-col justify-between h-full">
                    <div>
                        ...
                        <div className="mt-1">
                            ...
                            <span className="text-xs font-bold text-slate-400 block mt-1">{subLabel}: {subValue}</span>
                        </div>
                    </div>
                    ...
                    <div className="mt-4 flex items-center gap-1 text-[10px] font-bold">
                        ...
                        <span className={`${isPositive ? 'text-emerald-600 bg-emerald-50' : 'text-red-600 bg-red-50'} flex items-center gap-1 px-2 py-0.5`}>
                            ...
                            {isPositive ? 'Tăng trưởng' : 'Suy giảm'}
                        </span>
                    </div>
                </div>
            </div>
...
const UNIT_DB = [
    ...
    { madv: "392", donvi: "VNPT Tân Phú Đông", viettat: "TPD" },
    ...
    { madv: "401", donvi: "VNPT Chợ Gạo (DTP)", viettat: "CGO_DTP" },
];
...
export default function DashboardNetAdd() {
    ...
    const globalStats = useMemo(() => {
        if (rawData.length === 0) return {
            fiber: { ptm: 0, kp: 0, tl: 0, net: 0 },
            mytv: { ptm: 0, kp: 0, tl: 0, net: 0 }
        };
    ...
    return (
        <div className={`min-h-screen bg-slate-50 font-sans text-slate-800 pb-10 ${isFullScreen ? 'fixed inset-0 z-50 overflow-auto' : ''}`}>
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between h-16 items-center">
                        ...
                        <div className="flex bg-slate-100 p-1 rounded-none">
                            <button onClick={() => setActiveTab('overview')} className={`flex items-center gap-2 px-4 py-1.5 text-xs font-bold uppercase transition-all rounded-none ${activeTab === 'overview' ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                                <LayoutDashboard size={14} /> Toàn cảnh
                            </button>
                            <button onClick={() => setActiveTab('analysis')} className={`flex items-center gap-2 px-4 py-1.5 text-xs font-bold uppercase transition-all rounded-none ${activeTab === 'analysis' ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                                <BarChart2 size={14} /> Phân tích
                            </button>
                            <button onClick={() => setActiveTab('map')} className={`flex items-center gap-2 px-4 py-1.5 text-xs font-bold uppercase transition-all rounded-none ${activeTab === 'map' ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
                ...
                <div>
                    <h3 className="font-bold text-slate-600 mb-3 flex items-center gap-2 uppercase tracking-wide text-xs">
                        <Wifi size={14} className="text-blue-600" />
                        Dịch vụ Fiber Internet (58)
                    </h3>
                ...
                <div className="bg-white border border-slate-200 p-4 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4 sticky top-16 z-20">
                    ...
                    <div className="flex flex-wrap gap-2 w-full md:w-auto items-center">
                        <div className="flex bg-slate-100 p-1 rounded-none">
                            <button onClick={() => setFilterStatus('all')} className={`px-3 py-1.5 text-xs font-bold ${filterStatus === 'all' ? 'bg-white shadow text-slate-800' : 'text-slate-500'}`}>Tất cả</button>
                            <button onClick={() => setFilterStatus('achieved')} className={`px-3 py-1.5 text-xs font-bold ${filterStatus === 'achieved' ? 'bg-white shadow text-emerald-700' : 'text-slate-500'}`}>Tăng trưởng (+)</button>
                            <button onClick={() => setFilterStatus('not_reached')} className={`px-3 py-1.5 text-xs font-bold ${filterStatus === 'not_reached' ? 'bg-white shadow text-red-700' : 'text-slate-500'}`}>Suy giảm (-)</button>
                        </div>
                        ...
                        <div className="lg:col-span-2 bg-white p-5 border border-slate-200 shadow-sm h-[500px] flex flex-col">
                            ...
                            <div className="flex-1 w-full min-h-0">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart
                                        data={processedData.length > 15 ? processedData.slice(0, 15) : processedData}
                                    ...
                                    >      <CartesianGrid strokeDasharray="3 3" vertical={false} />
                                        ...
                                        <Bar dataKey="netFiber" name="Net Fiber" fill="#6366f1" barSize={12} radius={[2, 2, 0, 0]} onClick={(data) => setSelectedUnit(data)}>
                                            <LabelList dataKey="netFiber" position="top" style={{ fontSize: '9px', fill: '#64748b' }} />
                                        </Bar>
                                        <Bar dataKey="netMytv" name="Net MyTV" fill="#a855f7" barSize={12} radius={[2, 2, 0, 0]} onClick={(data) => setSelectedUnit(data)}>
                                            <LabelList dataKey="netMytv" position="top" style={{ fontSize: '9px', fill: '#64748b' }} />
                                        </Bar>
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                                ...
                                <>
                                    ...
                                    <div className="space-y-4 mt-6">
                                        ...
                                        <div className="bg-slate-50 p-3 rounded border border-slate-100">
                                            <h4 className="text-xs font-bold text-blue-600 uppercase mb-2 flex justify-between">
                                            ...
                                            <div className="grid grid-cols-3 gap-2 text-center">
                                                <div><div className="text-sm font-bold text-slate-700">{selectedUnit.ptmFiber}</div><div className="text-[9px] text-slate-500">PTM</div></div>
                                                <div><div className="text-sm font-bold text-slate-700">{selectedUnit.kpFiber}</div><div className="text-[9px] text-slate-500">KP</div></div>
                                                <div><div className="text-sm font-bold text-red-500">{selectedUnit.tlFiber}</div><div className="text-[9px] text-slate-500">Hủy</div></div>
                                            </div>
                                        </div>

                                        {/* MYTV STATS */}
                                        <div className="bg-slate-50 p-3 rounded border border-slate-100">
                                            <h4 className="text-xs font-bold text-purple-600 uppercase mb-2 flex justify-between">
                                            ...
                                            <div className="grid grid-cols-3 gap-2 text-center">
                                                <div><div className="text-sm font-bold text-slate-700">{selectedUnit.ptmMytv}</div><div className="text-[9px] text-slate-500">PTM</div></div>
                                                <div><div className="text-sm font-bold text-slate-700">{selectedUnit.kpMytv}</div><div className="text-[9px] text-slate-500">KP</div></div>
                                                <div><div className="text-sm font-bold text-red-500">{selectedUnit.tlMytv}</div><div className="text-[9px] text-slate-500">Hủy</div></div>
                                            </div>
                                        </div>
                                    </div>
                                </>
                            ...
                            {selectedUnit ? (
                                <>
                                    ...
                                    <div className="h-40 mt-4 pt-4 border-t border-slate-100">
                                        <h4 className="text-xs font-bold text-slate-400 uppercase text-center mb-2">Tỷ trọng PTM Fiber / MyTV</h4>
